...
public class AutoKiller extends JavaPlugin {
  ...
  public void logCheat(UUID uuid, String cheat, int vl) {
    ...
    String alert =
        cheat.endsWith("Click-Speed") ? config.getClickSpeedAlert() : config.getNormalAlert();
    alert =
        alert
    ...
    if (config.getCustomCommand()) {
      String command =
          config.getAlertCommand().replace("%player%", player.getName()).replace("%alert%", alert);
      ...
      if (players.isOp() || players.hasPermission("autokiller.staff")) {
        players.sendMessage(
            ChatColor.translateAlternateColorCodes(
                '&',
                "&8[&6AutoKiller&8] &cAuto-Banning player &e" + bannedName + " &cfor &e" + reason));
      }
    }
    this.getServer()
        .getScheduler()
        .runTaskLater(
            ...
            () -> {
              for (UUID uuids : playersToBeBannedUnlessCanceledYay) {
                ...
                Bukkit.getServer()
                    .dispatchCommand(Bukkit.getServer().getConsoleSender(), banCommand);
                ...
                String date = df.format(dateobj.getTime());
                logFile.write(
                    player.getPlayer(), "[" + date + "] " + player.getName() + " was AutoBanned");
              }
            },
            20L);
    this.getServer()
        .getScheduler()
        .runTaskLater(this, () -> playersToBeBannedUnlessCanceledYay.clear(), 20L * 2);
  }
